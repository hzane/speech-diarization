...
def load_gtcrn(checkpoint_path: str, device: torch.device) -> torch.nn.Module:
    """加载 GTCRN 模型并加载权重"""
...
class AudioEnhancer(nn.Module):
    def __init__(self, core_model: nn.Module, n_fft: int = 512, hop_length: int = 256, win_length: int = 512):
        super().__init__()
...
def enhance_audio(
    ...
    hop_length: int = 256
    win_length: int = 512
    window = torch.hann_window(win_length, device=device).pow(0.5)
    ...
    input = torch.view_as_real(stft) # [1, F, T, 2]

    with torch.no_grad():
        y = model(input) # [B, F, T, 2]

    ycomplex = torch.view_as_complex(y.contiguous())
    ...
    return waveform.cpu() # [B, T]
'''

def enhance_audio_chunked(
    enhancer: AudioEnhancer, # <--- 接收封装好的 enhancer 对象
    waveform: torch.Tensor,
    sample_rate: int,
    ...
    if num_samples <= chunk_size:
        return enhancer(waveform.squeeze(0))
    ...
    for chunk in track(waveform_chunks, description="Enhancing chunks"):
        # --- 核心调用变得非常简洁 ---
        # enhancer 对象内部处理了所有 STFT, 模型推理, iSTFT 和设备移动
        enhanced_chunk = enhancer(chunk)
...
if __name__ == '__main__':
    ...
    model = load_gtcrn('models.gtcrn/model_trained_on_dns3.tar', device=device)
    enhancer = AudioEnhancer(core_model=model)
